...
-(void)shareImgTextToQZone:(NSMutableArray *)inArguments {
    ...
    if (inArguments.count >=2) {
        ...
        NSArray *previewImageUrlArray = nil;
        
        previewImageUrlArray = [dict objectForKey:@"imageUrl"];
        
        if (IS_NSMutableArray(previewImageUrlArray)) {
            
            QQApiNewsObject *newsObj = Nil;
            NSString *previewImageUrl = [previewImageUrlArray objectAtIndex:0];
            //如果是网络图片
            if ([previewImageUrl hasPrefix:@"http://"]) {
                
                if (1 == [previewImageUrlArray count]) {
                    ...
                    newsObj = [QQApiNewsObject
                               ...
                               description:description
                               previewImageURL:STRTOURL(previewImageUrl)
                               imageUrlArray:previewImageUrlArray];
                }
                ...
                if (image) {
                    ...
                    if (1 == [previewImageUrlArray count]) {
                        ...
                        for (NSString *path in previewImageUrlArray) {
...
- (void)cbShare {
    NSLog(@"lgf-->uexQQApi-->jsStr=%@",self.cbShareStr);
...
- (void)tencentDidLogin {
    ...
    if (_tencentOAuth.accessToken && 0 != [_tencentOAuth.accessToken length]) {
        ...
        //self.cbQQLoginStr = [NSString stringWithFormat:@"uexQQ.cbLogin(0,0,0);"];
        self.cbQQLoginStr = [NSString stringWithFormat:@"{\"ret\":0,\"openid\":\"%@\",\"access_token\":\"%@\"}",_tencentOAuth.openId,_tencentOAuth.accessToken];
        
    } else {
        
        //self.cbQQLoginStr = [NSString stringWithFormat:@"uexQQApi.cbLogin(0,0,1);"];
